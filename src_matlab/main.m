...
for i_run = 0:n_runs-1
  ...
  results = zeros(3,5,length(x)); 

  % Main loop
  for ii = 1:length(x)
    jj = x(ii);
    [results(1, 1, ii), results(1, 2, ii), results(1, 3, ii), ...
      results(1, 4, ii), results(1, 5, ii)] = pipeline1(data(1:jj,:),...
      classes(1:jj), number_of_permutations, k_fold1, k_fold2);
    [results(2, 1, ii), results(2, 2, ii), results(2, 3, ii), ...
      results(2, 4, ii), results(2, 5, ii)] = pipeline2(data(1:jj,:), ...
      classes(1:jj), 0.5, number_of_permutations, k_fold1, k_fold2);
    [results(3, 1, ii), results(3, 2, ii), results(3, 3, ii), ...
      results(3, 4, ii), results(3, 5, ii)] = pipeline3(data(1:jj,:), ...
